...
Register-ArgumentCompleter -Native -CommandName 'intercept-bounce' -ScriptBlock {
    ...
    $completions = @(switch ($command) {
        'intercept-bounce' {
            ...
            [CompletionResult]::new('--ignore-key', '--ignore-key', [CompletionResultType]::ParameterName, 'Key codes or names to ignore (never debounce). Example: `--ignore-key 114` or `--ignore-key KEY_VOLUMEDOWN`')
