...
3.  **Bounce Check:** When a new **key event** arrives (press or release only; repeats are ignored):
    ...
    *   If the time difference is *less than* the configured `--debounce-time` threshold, the event is considered a bounce/chatter and is **dropped**.
...
4.  **Event Logging:** If `--log-all-events` or `--log-bounces` is enabled, formats and prints the relevant event details to standard error.
